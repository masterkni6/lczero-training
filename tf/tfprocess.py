...
class TFProcess:
    ...
    def strategy_calculate_test_summaries_inner_loop(self, x, y, z, q, m):
        policy_loss, value_loss, mse_loss, policy_accuracy, value_accuracy, moves_left_mean_error, policy_entropy, policy_ul = self.strategy.experimental_run_v2(self.calculate_test_summaries_inner_loop, args=(
        x, y, z, q, m))
        policy_loss = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, policy_loss, axis=None)
        value_loss = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, value_loss, axis=None)
        mse_loss = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, mse_loss, axis=None)
        ...
        moves_left_mean_error = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, mse_loss, axis=None)
        policy_entropy = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, policy_accuracy, axis=None)
        policy_ul = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, value_accuracy, axis=None)
        return policy_loss, value_loss, mse_loss, policy_accuracy, value_accuracy, moves_left_mean_error, policy_entropy, policy_ul
    ...
    def calculate_test_summaries_v2(self, test_batches, steps):
        ...
        for _ in range(0, test_batches):
            ...
            if self.strategy is not None:
                policy_loss, value_loss, mse_loss, policy_accuracy, value_accuracy, moves_left_mean_error, policy_entropy, policy_ul  = self.strategy_calculate_test_summaries_inner_loop(
                x, y, z, q, m)
            else:
