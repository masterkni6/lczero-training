...
def get_latest_chunks(path, num_chunks, allow_less, sort_key_fn, dump_file=None):
    ...
    if False and dump_file is not None and os.path.isfile(dump_file):
        ...
        if os.path.isfile(dump_file):
            with open(dump_file, "rb") as fp:   # Unpickling
        ...
        print(f"{dump_file} doesnt exist")
        chunks = get_all_chunks(path)
        print(len(chunks), num_chunks, len(chunks) < num_chunks)
        if len(chunks) < num_chunks:
            if allow_less:
                print("sorting {} chunks...".format(len(chunks)),
                      end='',
...
def main(cmd):
    ...
    if 'input_test' in cfg['dataset']:
        train_chunks = get_latest_chunks(cfg['dataset']['input_train'],
                                         num_train, allow_less, sort_key_fn, "train_dump")
        test_chunks = get_latest_chunks(cfg['dataset']['input_test'], num_test,
                                        allow_less, sort_key_fn, "test_dump")
    else:
